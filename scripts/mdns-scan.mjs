...
const parse = (buf) => {
  ...
  for (let i = 0; i < counts[1] + counts[2] + counts[3]; i++) {
    ...
    let value
    if (type === 12) value = readName(buf, rdata)[0] // PTR
    else if (type === 1) value = Array.from(buf.subarray(rdata, rdata + 4)).join(".") // A
    else if (type === 33) value = `${readName(buf, rdata + 6)[0]}:${buf.readUInt16BE(rdata + 4)}` // SRV
...
socket.on("message", (message) => {
  ...
  for (const { name, type, value } of records) {
    if (type === 12 && name === "_services._dns-sd._udp.local") serviceTypes.add(value)
    else if (type === 12) remember(instances, name, value)
    else remember(rdata, name, `${type === 1 ? "A" : type === 33 ? "SRV" : "TXT"} ${value}`)
  }
})
...
const report = () => {
  ...
  for (const [type, names] of [...instances].sort()) {
    for (const name of names) {
      console.log(`  ${type}`)
      console.log(`    ${name}`)
      for (const record of rdata.get(name) ?? []) console.log(`      ${record}`)
    }
  }
...
socket.bind(UNICAST_REPLY ? 0 : PORT, PINNED && UNICAST_REPLY ? IFACE : undefined, () => {
  ...
  setTimeout(() => {
    if (serviceTypes.size > 0) socket.send(query([...serviceTypes]), PORT, MCAST)
  }, 2500)
