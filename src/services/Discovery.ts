...
const looksLikeLg = (headers: Record<string, string>): boolean =>
  Object.entries(headers).some(
    ([key, value]) =>
      (key === "st" || key === "usn" || key === "server" || key === "location") &&
      /lge?[-_ ]?com|webos|lg electronics|lgsmarttv/i.test(value)
  )

const rawSearch = (timeout: Duration.Duration) =>
  Effect.async<Map<string, Record<string, string>>, DiscoveryFailed>((resume) => {
    const found = new Map<string, Record<string, string>>()
    const socket = dgram.createSocket({ type: "udp4", reuseAddr: true })

    let settled = false
    const finish = (result: Effect.Effect<Map<string, Record<string, string>>, DiscoveryFailed>) => {
      if (settled) return
      settled = true
      clearTimeout(timer)
      try {
    ...
    const timer = setTimeout(() => finish(Effect.succeed(found)), Duration.toMillis(timeout))
...
const describe = (location: string | undefined): Effect.Effect<Description> =>
  location === undefined
    ...
    : Effect.tryPromise({
        ...
        catch: () => new DiscoveryFailed({ detail: "could not read device description" })
      }).pipe(
