...
import { SettingsUnreadable, TvNotConfigured } from "../domain/errors.js"
...
export interface SessionApi {
  ...
  readonly clientKey: Effect.Effect<Option.Option<string>, TvNotConfigured | SettingsUnreadable>
...
export class Session extends Context.Tag("lgtv/Session")<Session, SessionApi>() {}

const envOption = (name: string) => Option.fromNullable(process.env[name]).pipe(
  Option.filter((value) => value.trim().length > 0)
)

export const layer = (options: GlobalOptions): Layer.Layer<Session, never, Settings> =>
  Layer.effect(
    Session,
    Effect.gen(function* () {
      ...
      const host = Effect.gen(function* () {
        const explicit = Option.orElse(options.host, () => envOption("LGTV_HOST"))
       ...
       */
      const sslFor = (h: string): Effect.Effect<boolean, SettingsUnreadable> => {
        if (Option.isSome(options.ssl)) return Effect.succeed(options.ssl.value)
      ...
      const wsUrl = Effect.gen(function* () {
        ...
        const resolvedPort = Option.getOrElse(port, () => (secure ? 3001 : 3000))
        return `${secure ? "wss" : "ws"}://${h}:${resolvedPort}`
      })

      const clientKey = Effect.flatMap(host, (h) =>
        Effect.map(settings.deviceFor(h), Option.flatMap((d) => Option.fromNullable(d.clientKey)))
      )
      ...
      const mac = Effect.gen(function* () {
        ...
        const stored = Option.flatMap(device, (d) => Option.fromNullable(d.mac))
      ...
      const rememberKey = (h: string, key: string) =>
        Effect.flatMap(sslFor(h), (secure) =>
          Effect.asVoid(settings.rememberDevice(h, { clientKey: key, ssl: secure }))
        )
