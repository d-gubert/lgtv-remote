...
import { Console, Context, Duration, Effect, Option, Stream } from "effect";

import { Session } from "./Session.js";
import type { SettingsUnreadable, TvNotConfigured } from "../domain/errors.js";
import {
  PairingFailed,
  SsapFailed,
  TvUnreachable,
...
const call = <A>(
  ...
  Effect.tryPromise({ try: work, catch: (error) => error }).pipe(
    Effect.catchAll((error) =>
      Ssap.isSsapError(error)
        ? Effect.fail(toDomainError(error))
        : Effect.die(error),
    ),
  );
...
export interface PointerInput {
  ...
  readonly move: (
    ...
    dy: number,
    drag: boolean,
  ) => Effect.Effect<void, TvUnreachable>;
  readonly scroll: (
    dx: number,
    dy: number,
  ) => Effect.Effect<void, TvUnreachable>;
...
export interface Tv {
  ...
  readonly subscribe: (
    uri: string,
  ) => Stream.Stream<AnyPayload, SsapFailed | TvUnreachable>;
...
export const connect = (
  ...
  Effect.gen(function* () {
    ...
    const announce = () =>
      Effect.runSync(
        Console.error("→ Accept the pairing request on your TV screen…"),
      );
      ...
      restore(
        call((signal) =>
          Ssap.connect({
            ...
            ...(options.announcePairing === false
              ? {}
              : { onPairingPrompt: announce }),
          }),
        ),
      ).pipe(
        Effect.tap((open) =>
    ...
    const impossible = {
      PairingFailed: Effect.die,
      UnexpectedResponse: Effect.die,
    };
      ...
      Effect.tryPromise({ try: work, catch: (error) => error }).pipe(
        Effect.catchAll((error) =>
          error instanceof Ssap.TvUnreachable
            ? Effect.fail(
                new TvUnreachable({ url: error.url, cause: error.cause }),
              )
            : Effect.die(error),
        ),
      );
    ...
    const pointer = Effect.map(
      call(() => connection.pointer()).pipe(
        Effect.catchTag("PairingFailed", Effect.die),
      ),
      (input): PointerInput => ({
