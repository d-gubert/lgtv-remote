...
export const SettingsFile = Schema.Struct({
  ...
  devices: Schema.optional(
    Schema.Record({ key: Schema.String, value: Device }),
  ),
});
...
export class Settings extends Effect.Service<Settings>()("lgtv/Settings", {
  effect: Effect.gen(function* () {
    ...
    const read: Effect.Effect<SettingsFile, SettingsUnreadable> = Effect.gen(
      function* () {
        ...
        const text = yield* fs
          ...
          .pipe(
            Effect.mapError(
              (cause) =>
                new SettingsUnreadable({ path: file, detail: cause.message }),
            ),
          );
        const parsed = yield* Effect.try({
          try: () => YAML.parse(text) as unknown,
          catch: () =>
            new SettingsUnreadable({ path: file, detail: "not valid YAML" }),
        });
    ...
    const write = (
      next: SettingsFile,
    ): Effect.Effect<SettingsFile, SettingsUnreadable> =>
      Effect.gen(function* () {
    ...
    const forget = (host: string) =>
      update((s) => {
        ...
        const nextDefault =
          s.defaultHost === host ? remaining[0] : s.defaultHost;
        return {
          devices: rest,
