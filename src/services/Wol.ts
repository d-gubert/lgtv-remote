...
export const parseMac = (raw: string): Either.Either<string, BadInput> => {
  ...
  if (compact.length !== 12 || !/^[0-9a-f]{12}$/i.test(compact)) {
    return Either.left(
      new BadInput({ detail: `"${raw}" is not a MAC address (expected 6 hex pairs).` })
    )
  }
  const pairs = compact.match(/.{2}/g) ?? []
  if (!pairs.every((p) => HEX_PAIR.test(p))) {
    return Either.left(new BadInput({ detail: `"${raw}" is not a MAC address.` }))
  }
...
const subnetBroadcast = (host: string | undefined): string | undefined => {
  ...
  if (octets.length !== 4 || octets.some((o) => !/^\d{1,3}$/.test(o))) return undefined
...
export const wake = (
  mac: string,
  options: { readonly host?: string | undefined; readonly repeat?: number } = {}
  ...
  Effect.async<ReadonlyArray<string>, WakeFailed>((resume) => {
    ...
    let settled = false
    const finish = (result: Effect.Effect<ReadonlyArray<string>, WakeFailed>) => {
      if (settled) return
      settled = true
      try {
        socket.close()
    ...
    socket.once("error", (cause) => finish(Effect.fail(new WakeFailed({ detail: cause.message }))))
    ...
    socket.bind(() => {
      try {
        ...
        finish(
          Effect.fail(
            new WakeFailed({ detail: cause instanceof Error ? cause.message : String(cause) })
          )
        )
        return
      }
      ...
      for (let attempt = 0; attempt < repeat; attempt += 1) {
        for (const address of addresses) {
          for (const port of ports) {
            socket.send(packet, 0, packet.length, port, address, (cause) => {
