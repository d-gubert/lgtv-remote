		...
		Effect.gen(function* () {
			if (names.length === 0) {
				return yield* Effect.fail(
					new BadInput({
						detail: "Give at least one button name, e.g. `lgtv key HOME`.",
					}),
				);
			}
			const resolved = names.map((name) => ({
				name,
				button: resolveButton(name),
			}));
					...
					yield* Effect.forEach(
						...
						(button, index) =>
							index === 0
								...
								: Effect.zipRight(
										Effect.sleep(Duration.millis(gap)),
										pointer.button(button),
									),
						{ discard: true },
					);
...
export const cursorCommand = Command.make("cursor", {}, () =>
	Effect.fail(
		new BadInput({ detail: "Usage: lgtv cursor <move|click|scroll>" }),
	),
).pipe(
	...
	({ text, submit, replace }) =>
		withTv((tv) =>
			Effect.gen(function* () {
				if (replace) {
					yield* tv
						.request(Uri.deleteCharacters, { count: 255 })
						.pipe(Effect.ignore);
				}
				yield* tv.request(Uri.insertText, { text, replace });
				if (submit) yield* tv.request(Uri.sendEnterKey);
				yield* ok(
					`Typed ${JSON.stringify(text)}${submit ? " and pressed Enter" : ""}`,
				);
			}),
		),
...
const mediaActions: ReadonlyArray<
...
export const mediaCommand = Command.make("media", { action }, ({ action }) =>
	withTv((tv) =>
		Effect.zipRight(tv.request(action.uri), ok(`Sent ${action.name}`)),
	),
