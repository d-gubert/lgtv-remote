...
export const discoverCommand = Command.make("discover", { seconds }, ({ seconds }) =>
  Effect.gen(function* () {
...
).pipe(Command.withDescription("Find LG webOS TVs on the local network (SSDP)"))
      ...
      [
        ...
        table(
          ...
          devices.map(([host, device]) => [
            ...
            device.ssl === undefined ? "—" : device.ssl ? "on" : "off"
          ])
        )
      ].join("\n"),
...
const setHost = Command.make("set-host", { host: Args.text({ name: "ip-or-hostname" }) }, ({ host }) =>
  Effect.gen(function* () {
    const settings = yield* Settings
...
const setMac = Command.make("set-mac", { mac: Args.text({ name: "mac-address" }) }, ({ mac }) =>
  Effect.gen(function* () {
    const settings = yield* Settings
    const session = yield* Session
...
const setSsl = Command.make(
  "set-ssl",
  { ssl: Args.choice([["on", true], ["off", false]], { name: "on-or-off" }) },
  ({ ssl }) =>
    Effect.gen(function* () {
      const settings = yield* Settings
      const session = yield* Session
      ...
      yield* ok(`${cyan(host)} will use the ${ssl ? "secure" : "plain"} port by default`)
    })
).pipe(Command.withDescription("Choose the transport used for a TV when no flag is given"))
