...
export const channelCommand = Command.make("channel", {}, () =>
  Effect.fail(new BadInput({ detail: "Usage: lgtv channel <current|list|up|down|set>" }))
).pipe(
  Command.withDescription("Browse and change channels"),
  Command.withSubcommands([channelCurrent, channelList, channelUp, channelDown, channelSet])
)
