...
const includeSystem = Options.boolean("all").pipe(
  Options.withDescription("Include system apps, not just the ones on the launcher")
)
...
const appList = Command.make("list", { includeSystem }, ({ includeSystem }) =>
  withTv((tv) =>
    Effect.gen(function* () {
      ...
      const label = (app: { readonly id: string; readonly title?: string | undefined }) =>
        (app.title ?? app.id).toLowerCase()
      const sorted = [...visible].sort((a, b) => label(a).localeCompare(label(b)))
      yield* emit(
        table(
          ["ID", "TITLE"],
...
const appCurrent = Command.make("current", {}, () =>
  withTv((tv) =>
    Effect.gen(function* () {
      ...
      yield* emit(app.appId === undefined ? dim("nothing running") : cyan(app.appId), app)
    })
  )
...
const appLaunch = Command.make("launch", { appId, contentId }, ({ appId, contentId }) =>
  withTv((tv) =>
    Effect.zipRight(
...
const appClose = Command.make("close", { appId }, ({ appId }) =>
  withTv((tv) => Effect.zipRight(tv.request(Uri.closeApp, { id: appId }), ok(`Closed ${appId}`)))
).pipe(Command.withDescription("Close a running app"))

export const appCommand = Command.make("app", {}, () =>
  Effect.fail(new BadInput({ detail: "Usage: lgtv app <list|current|launch|close>" }))
).pipe(
...
const inputList = Command.make("list", {}, () =>
  withTv((tv) =>
    Effect.gen(function* () {
      const { devices } = yield* tv.requestAs(Uri.listInputs, ExternalInputList)
      yield* emit(
        table(
          ["ID", "LABEL", "CONNECTED"],
          devices.map((device) => [
