...
const search = Options.text("search").pipe(
  Options.withDescription(
    "Open the search screen for this query instead of a link",
  ),
  Options.optional,
);
...
const start = Options.integer("start").pipe(
  Options.withDescription(
    "Start this many seconds in, overriding any t= in the link",
  ),
  Options.optional,
);
...
const chooseTarget = (
  ...
  return Option.isSome(link)
    ...
    : Either.left(
        new BadInput({
          detail: `Give a YouTube link or video id, or --search "planet earth".`,
        }),
      );
};
...
export const youtubeCommand = Command.make(
    ...
    Effect.gen(function* () {
      if (Option.isSome(search) && Option.isSome(start)) {
        return yield* Effect.fail(
          new BadInput({
            detail: "--start applies to a video, not to a search.",
          }),
        );
      }
      ...
      const target = Option.isSome(start)
        ? { ...parsed, startSeconds: start.value }
        : parsed;
      ...
      yield* withTv((tv) =>
        Effect.zipRight(
          ...
          emit(
            `${green("✓")} Opening ${cyan(describeTarget(target))}\n${dim(deepLink)}`,
            {
              appId,
  ...
  Command.withDescription(
    "Open a YouTube video, playlist, or search in the TV's YouTube app",
  ),
);
