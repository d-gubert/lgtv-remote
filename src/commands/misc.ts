...
export const toastCommand = Command.make("toast", { message }, ({ message }) =>
  withTv((tv) =>
    Effect.zipRight(tv.request(Uri.createToast, { message }), ok("Notification shown"))
  )
...
export const rawCommand = Command.make("raw", { uri, payload }, ({ uri, payload }) =>
  Effect.gen(function* () {
    const body = yield* Option.match(payload, {
      ...
      onSome: (text) =>
        Effect.try({
          ...
          catch: () => new BadInput({ detail: `--payload is not valid JSON: ${text}` })
        })
    })
...
).pipe(Command.withDescription("Send an arbitrary ssap:// request and print the reply"))

// ---- watch ----------------------------------------------------------------

const topics: ReadonlyArray<[string, { readonly name: string; readonly uri: string }]> = [
...
export const watchCommand = Command.make("watch", { topic }, ({ topic }) =>
  withTv((tv) =>
    Effect.gen(function* () {
      ...
      yield* Console.error(dim(`Watching ${topic.name} — press Ctrl-C to stop.`))
      yield* Stream.runForEach(tv.subscribe(topic.uri), (update) =>
        Console.log(
          session.json
            ? JSON.stringify(update)
            : `${dim(new Date().toISOString().slice(11, 19))} ${cyan(
                JSON.stringify(update, (key, value) => (key === "subscribed" ? undefined : value))
              )}`
        )
      )
    })
