...
const commands = Args.text({ name: "command" }).pipe(
  Args.withDescription('One lgtv command without the leading "lgtv", e.g. "volume set 12"'),
  Args.repeated
)
  ...
  Effect.gen(function* () {
    ...
    if (lines.every((line) => line.trim() === "")) {
      return yield* Effect.fail(
        new BadInput({ detail: `Give at least one command, e.g. lgtv run "volume set 12".` })
      )
    }
