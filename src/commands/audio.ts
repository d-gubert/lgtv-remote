...
const readStatus = (tv: Tv) =>
	tv
		...
		.pipe(
			Effect.catchTag("SsapFailed", () =>
				tv.requestAs(Uri.getVolume, AudioStatus),
			),
		);
		...
		Effect.gen(function* () {
			...
			yield* emit(
				`${cyan(String(status.volume ?? "?"))}${status.muted === true ? dim(" (muted)") : ""}${
					status.soundOutput === undefined
						? ""
						: dim(` · out: ${status.soundOutput}`)
				}`,
				status,
			);
		}),
		...
		Effect.gen(function* () {
			if (steps < 1) {
				return yield* Effect.fail(
					new BadInput({
						detail: `Step count must be at least 1 (got ${steps}).`,
					}),
				);
			}
			yield* withTv((tv) =>
				Effect.zipRight(
					Effect.repeatN(tv.request(uri), steps - 1),
					ok(`${label} ×${steps}`),
				),
			);
		}),
...
const level = Args.integer({ name: "level" }).pipe(
	Args.withDescription("Target volume, 0-100"),
);
...
const volumeSet = Command.make("set", { level }, ({ level }) =>
	Effect.gen(function* () {
		if (level < 0 || level > 100) {
			return yield* Effect.fail(
				new BadInput({
					detail: `Volume must be between 0 and 100 (got ${level}).`,
				}),
			);
		}
		yield* withTv((tv) =>
			Effect.zipRight(
				tv.request(Uri.setVolume, { volume: level }),
...
export const muteCommand = Command.make(
	"mute",
	{ state: muteState },
	({ state }) =>
		withTv((tv) =>
