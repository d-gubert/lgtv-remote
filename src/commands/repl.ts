    ...
    words.length <= 1
      ? [
          ...
          ...subcommands.flatMap((command) => [
            ...Descriptor.getNames(command.descriptor),
          ]),
        ]
      : (() => {
          const head = words[0] ?? "";
...
const echoing = (tv: Tv, heard: Ref.Ref<boolean>): Tv => {
  ...
  return {
    ...
    subscribe: (uri) =>
      Stream.tap(tv.subscribe(uri), () => Ref.set(heard, true)),
...
export const runRepl = (
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const classify = (
        outcome: Exit.Exit<void, LineError>,
      ...
      if (banner)
        yield* Console.error(
          `Connected to ${tv.host}. Type a command, or "help".`,
        );

      const step: Effect.Effect<boolean, never, DispatchContext | Session> =
        Effect.gen(function* () {
...
export const replCommand = Command.make("repl", {}, () =>
  ...
  Command.withDescription("Drive many commands over one long-lived connection"),
);
