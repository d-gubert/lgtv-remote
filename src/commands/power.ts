...
const waitFor = Options.integer("wait").pipe(
  Options.withDescription(
    "Seconds to wait for the TV to come back before giving up",
  ),
...
export const onCommand = Command.make("on", { waitFor }, ({ waitFor }) =>
  Effect.gen(function* () {
    ...
    yield* Console.error(
      dim(`Waiting up to ${waitFor}s for ${host} to come online…`),
    );
    const online = yield* Effect.scoped(
      connect({ announcePairing: false }),
    ).pipe(
      Effect.as(true),
      Effect.retry(
  ...
  Command.withDescription("Power the TV on with a Wake-on-LAN magic packet"),
);

export const offCommand = Command.make("off", {}, () =>
  withTv((tv) =>
    Effect.zipRight(tv.request(Uri.turnOff), ok(`${tv.host} going to standby`)),
  ),
...
export const screenCommand = Command.make(
  "screen",
  { state: screenTarget },
  ({ state }) =>
    withTv((tv) =>
  ...
  Command.withDescription("Blank or restore the panel (audio keeps playing)"),
);
...
export const statusCommand = Command.make("status", {}, () =>
  withTv((tv) =>
    Effect.gen(function* () {
      ...
      const get = <A>(
        source: Option.Option<Record<string, unknown>>,
        key: string,
      ): A | undefined =>
        Option.isSome(source)
          ? (source.value[key] as A | undefined)
          : undefined;

      const volumeStatus =
        get<Record<string, unknown>>(volume, "volumeStatus") ?? {};
      const level =
        get<number>(volume, "volume") ??
        (volumeStatus.volume as number | undefined);
      const muted =
        get<boolean>(volume, "muted") ??
        (volumeStatus.muteStatus as boolean | undefined);
      ...
      const channelLine =
        data.channel === null
          ? []
          : [`channel   ${data.channelNumber ?? "?"} ${data.channel}`];
  ...
  Command.withDescription("Show power state, current app, volume and channel"),
);
