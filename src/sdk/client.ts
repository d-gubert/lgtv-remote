...
import { PairingFailed, SsapFailed, TvUnreachable, UnexpectedResponse } from "./errors.js"
...
const asFrame = (input: unknown): IncomingFrame | undefined => {
  ...
  if (frame["error"] !== undefined && typeof frame["error"] !== "string") return undefined
...
const openSocket = (url: string, timeoutMs: number, signal?: AbortSignal): Promise<WebSocket> =>
...
const sendText = (socket: WebSocket, url: string, data: string): Promise<void> =>
...
export interface Connection {
  ...
  readonly request: (uri: string, payload?: Payload, options?: CallOptions) => Promise<Payload>
   ...
   */
  readonly subscribe: (uri: string, listener: SubscriptionListener) => Subscription
...
export const connect = async (options: ConnectOptions): Promise<Connection> => {
  ...
  const awaitReply = (
    ...
    new Promise<Payload>((resolve, reject) => {
      ...
      const timer = setTimeout(
        () => settle(() => reject(new SsapFailed(uri, `no reply within ${callTimeoutMs / 1000}s`))),
        callTimeoutMs
      )
      const onAbort = () => settle(() => reject(abortReason(signal as AbortSignal)))
      signal?.addEventListener("abort", onAbort, { once: true })
  ...
  const request = (uri: string, payload?: Payload, callOptions?: CallOptions) => {
    const id = nextId()
    ...
    send({ id, type: "request", uri, ...(payload === undefined ? {} : { payload }) })
    return reply
  }
  ...
  const subscribe = (uri: string, listener: SubscriptionListener): Subscription => {
    const id = nextId()
    let live = true
    const close = () => {
      if (!live) return
  ...
  const openPointer = async (callOptions?: CallOptions): Promise<Pointer> => {
    const { socketPath } = await requestAs(Uri.pointerSocket, PointerSocket, undefined, callOptions)
  ...
  const handshake = new Promise<string>((resolve, reject) => {
    ...
    const timer = setTimeout(
      () => settle(() => reject(new PairingFailed(url, "timed out waiting for approval"))),
      Math.max(options.pairingTimeoutMs ?? 0, timeoutMs, MIN_PAIRING_TIMEOUT_MS)
    )
    const onAbort = () => settle(() => reject(abortReason(options.signal as AbortSignal)))
    ...
    pump.register(registerId, (delivery) => {
      ...
      if (frame.type === "error") {
        settle(() =>
          reject(new PairingFailed(url, frame.error ?? "the TV rejected the handshake"))
        )
        return
      }
  ...
  send({
    ...
    payload: {
      ...pairingManifest,
      ...(options.clientKey === undefined ? {} : { "client-key": options.clientKey })
    }
  })
