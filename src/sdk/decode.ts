...
const primitive = <A>(
  ...
  decode: (input, path) =>
    is(input)
      ? ok(input as A)
      : fail(path, `expected ${expected}, got ${typeName(input)}`),
});

export const string: Decoder<string> = primitive(
  "string",
  (i) => typeof i === "string",
);
...
export const boolean: Decoder<boolean> = primitive(
  "boolean",
  (i) => typeof i === "boolean",
);
...
export const record = <A>(
  value: Decoder<A>,
): Decoder<Readonly<Record<string, A>>> => ({
  decode: (input, path) => {
...
export type Fields = Readonly<
  Record<string, Decoder<unknown> | Optional<unknown>>
>;

type ValueOf<F> =
  F extends Optional<infer A> ? A : F extends Decoder<infer A> ? A : never;
...
const isOptional = (
  field: Decoder<unknown> | Optional<unknown>,
): field is Optional<unknown> => "decoder" in field;
...
export const payload: Decoder<Readonly<Record<string, unknown>>> =
  record(unknown);
