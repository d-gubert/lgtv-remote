...
const parseStart = (raw: string | null): number | undefined => {
  ...
  const part = (group: string | undefined) => (group === undefined ? 0 : Number(group))
...
const parseUrl = (input: string): Either.Either<YoutubeTarget, BadInput> => {
  ...
  if (!knownHosts.has(url.hostname.toLowerCase())) {
    return fail(`${url.hostname} is not a YouTube address — expected youtube.com or youtu.be`)
  }
  ...
  const fromPath =
    url.hostname.toLowerCase().endsWith("youtu.be")
      ? head
      : head !== undefined && pathVerbs.has(head)
        ? tail
        : undefined
  ...
  const startSeconds = parseStart(
    url.searchParams.get("t") ?? url.searchParams.get("start") ?? startFromHash(url.hash)
  )

  if (videoId !== undefined && !videoIdPattern.test(videoId)) {
    return fail(`"${videoId}" is not a valid YouTube video id (expected 11 characters)`)
  }
 ...
 */
export const parseYoutubeTarget = (input: string): Either.Either<YoutubeTarget, BadInput> => {
  ...
  if (!/[/?&#]/.test(text) && !text.includes(".")) {
    if (videoIdPattern.test(text)) return Either.right(build(text, undefined, undefined))
    if (listIdPattern.test(text)) return Either.right(build(undefined, text, undefined))
...
export const launchPayload = (
  ...
  return { id: appId, contentId: deepLink, params: { contentTarget: deepLink } }
}
