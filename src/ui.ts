 ...
 */
export const emit = (human: string, data: unknown): Effect.Effect<void, never, Session> =>
...
export const keyValue = (pairs: ReadonlyArray<readonly [string, string]>): string => {
  const width = Math.max(...pairs.map(([label]) => label.length))
  return pairs.map(([label, value]) => `${dim(`${label}:`.padEnd(width + 1))} ${value}`).join("\n")
}
