...
const toContractError = (error: unknown): ContractError =>
  isSsapError(error)
    ? new ContractError(
        error._tag satisfies ErrorKind,
        error._tag === "TvUnreachable"
    ...
    : new ContractError("Unknown", String(error), error)
  ...
  connect: async (options: ConnectOptions): Promise<Connection> => {
    const connection = await rethrow(() =>
      connect({
        ...
        ...(options.onClientKey === undefined ? {} : { onClientKey: options.onClientKey })
      })
    )
    ...
    return {
      ...
      request: (uri, payload) => rethrow(() => connection.request(uri, payload)) as Promise<Payload>,
      requestAs: (uri, shape, payload) =>
        rethrow(() => connection.requestAs(uri, decoders[shape], payload)) as Promise<Payload>,
