    ...
    const detail =
      fields.detail !== undefined
        ...
        : fields.cause instanceof Error
          ...
          : String(fields.cause ?? "")
    return new ContractError(tag as ErrorKind, detail, error)
  }
  return new ContractError("Unknown", String(error), error)
}

const runOrThrow = async <A, E>(effect: Effect.Effect<A, E, never>): Promise<A> => {
...
const sessionLayer = (options: ConnectOptions) => {
  ...
  const api: SessionApi = {
    ...
    wsUrl: Effect.succeed(`${secure ? "wss" : "ws"}://${options.host}:${options.port}`),
...
export const effectClient: ContractClient = {
  ...
  connect: async (options: ConnectOptions): Promise<Connection> => {
    ...
    const runScoped = <A, E>(effect: Effect.Effect<A, E, Session | Scope.Scope>): Promise<A> =>
      run(Scope.extend(effect, scope))
