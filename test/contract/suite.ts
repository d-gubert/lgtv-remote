  ...
  describe(`SSAP client contract [${client.name}]`, () => {
    ...
    describe("handshake", () => {
      it("pairs with a TV that has never seen us, and reports the key it was granted", async () => {
        ...
        const connection = await connect({ onClientKey: (key) => granted.push(key) })

        assert.match(connection.clientKey, /^key-/)
        assert.ok(tv.knownKeys.has(connection.clientKey), "the TV should consider the key paired")
        assert.equal(tv.promptCount(), 1, "the user should have been prompted exactly once")
        assert.deepEqual(granted, [connection.clientKey], "the key must be handed back to be stored")
      })
      ...
      it("reuses a key the TV already trusts, without prompting again", async () => {
        ...
        assert.equal(tv.promptCount(), 1, "a known key must not raise a second prompt")
        assert.equal(
          tv.registerPayloads.at(-1)?.["client-key"],
          first.clientKey,
          "the stored key has to be offered in the register frame"
        )
        assert.deepEqual(granted, [], "an unchanged key should not be written back")
      })
      ...
      it("closes the socket when the handshake fails", async () => {
        tv.setPairing("reject")
        await failsWith("PairingFailed", () => connect())
        await eventually(() => tv.openConnections() === 0, "the failed connection to be closed")
      })
    ...
    describe("requests", () => {
      ...
      it("surfaces an error frame as a refusal", async () => {
        const connection = await connect()
        const error = await failsWith("SsapFailed", () => connection.request(URI.unknown))
      ...
      it("gives up on a request the TV never answers", async () => {
        ...
        const error = await failsWith("SsapFailed", () => connection.request(URI.getVolume))
        assert.match(error.detail, /no reply/)
        assert.ok(Date.now() - started < 2000, "the timeout should fire promptly")
      })
      ...
      it("fails a request in flight when the socket drops, rather than hanging", async () => {
        ...
        const started = Date.now()
        const pending = failsWith("TvUnreachable", () => connection.request(URI.getVolume))
        await delay(50)
        tv.dropConnections()
        await pending
        assert.ok(Date.now() - started < 2000, "a dropped socket must fail waiters immediately")
      })
      ...
      it("routes concurrent replies to the caller that asked, even out of order", async () => {
        ...
        const apps = connection.request(URI.listApps).then((r) => (finished.push("apps"), r))
        const volume = connection.request(URI.getVolume).then((r) => (finished.push("volume"), r))
        const [appsResult, volumeResult] = await Promise.all([apps, volume])

        assert.deepEqual(finished, ["volume", "apps"], "the replies should arrive out of order")
        assert.ok("launchPoints" in appsResult, "the apps reply went to the wrong caller")
        assert.equal(volumeResult["volume"], 12, "the volume reply went to the wrong caller")
      })

      it("fails rather than hangs once the connection is closed", async () => {
        const connection = await connect()
        await connection.close()
        await failsWith("TvUnreachable", () => connection.request(URI.getVolume))
      })
    })
    ...
    describe("decoding", () => {
      it("decodes a reply into the shape the caller asked for", async () => {
        ...
        const status = await connection.requestAs(URI.getVolume, "VolumeStatus")
        assert.equal(status["volume"], 12)

        const apps = await connection.requestAs(URI.listApps, "LaunchPoints")
        const launchPoints = apps["launchPoints"] as ReadonlyArray<{ id: string }>
    ...
    describe("subscriptions", () => {
      ...
      it("leaves the connection usable after a subscription ends", async () => {
        ...
        const status = await connection.request(URI.getVolume)
        assert.equal(status["volume"], 12, "ending a subscription must not close the connection")
      })
      ...
      it("keeps two subscriptions on one connection apart", async () => {
        ...
        assert.equal(tv.subscriptionCount(), 2, "each subscribe needs its own id")
    ...
    describe("pointer input", () => {
      it("sends the Magic Remote frames verbatim", async () => {
        ...
        await eventually(() => tv.pointerFrames.length === 4, "all four pointer frames to arrive")
      ...
      it("marks a drag with down:1", async () => {
        ...
        await eventually(() => tv.pointerFrames.length === 1, "the move frame to arrive")
      ...
      it("closes the pointer channel along with the connection", async () => {
        ...
        await eventually(() => tv.openPointerConnections() === 1, "the pointer socket to open")

        await connection.close()
        await eventually(() => tv.openPointerConnections() === 0, "the pointer socket to close")
      })
    })
    ...
    describe("lifetime", () => {
      it("closes the socket when the connection is closed", async () => {
        ...
        await connection.close()
        await eventually(() => tv.openConnections() === 0, "the socket to be closed")
      })
