...
const MANIFEST_SHA256 =
  "11b0461f56af10bb0604753993292f5978b55cb1a9940232a3a2b0db685918d6";
...
export const runWireTests = (client: ContractClient): void => {
  describe(`SSAP wire transcript [${client.name}]`, () => {
    ...
    const frames = (): ReadonlyArray<Frame> =>
      tv.rawFrames.map((raw) => JSON.parse(raw) as Frame);
    const framesOfType = (type: string) =>
    ...
    it("puts the signed manifest on the wire unchanged", async () => {
      ...
      assert.equal(
        serialised.length,
        MANIFEST_BYTES,
        "the pairing manifest changed size",
      );
      assert.equal(
    ...
    it("frames a subscribe with no payload, sharing the request counter", async () => {
      ...
      assert.deepEqual(subscribe, [
        { id: "lgtv-1", type: "subscribe", uri: URI.getVolume },
      ]);
    });
  });
};
