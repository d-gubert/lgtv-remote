...
const bothMacs = {
  wiredInfo: { macAddress: WIRED },
  wifiInfo: { macAddress: WIFI },
};
...
describe("parseMac", () => {
  it("accepts the formats people actually paste", () => {
    for (const input of [
      ...
      assert.deepEqual(
        parseMac(input),
        Either.right("A1:B2:C3:D4:E5:F6"),
        input,
      );
    }
  });
...
describe("macForActiveInterface", () => {
  ...
  it("is undefined when the TV lists no MACs at all", () => {
    assert.equal(
      macForActiveInterface({}, { wifi: { state: "connected" } }),
      undefined,
    );
  });
});
...
describe("parseYoutubeTarget", () => {
  ...
  it("keeps a playlist, with or without a video alongside it", () => {
    assert.deepEqual(target(`https://www.youtube.com/playlist?list=${LIST}`), {
      listId: LIST,
    });
    assert.deepEqual(target(LIST), { listId: LIST });
    assert.deepEqual(
      target(`https://www.youtube.com/watch?v=${VIDEO}&list=${LIST}&index=2`),
      {
        videoId: VIDEO,
        listId: LIST,
      },
  ...
  it("reads the start time in the forms YouTube writes it", () => {
    ...
    assert.equal(
      seconds(`https://www.youtube.com/watch?v=${VIDEO}&t=1h2m3s`),
      3723,
    );
    assert.equal(
      seconds(`https://www.youtube.com/embed/${VIDEO}?start=45`),
      45,
    );
  ...
  it("ignores a timestamp it cannot read rather than refusing the video", () => {
    assert.deepEqual(
      target(`https://www.youtube.com/watch?v=${VIDEO}&t=soon`),
      { videoId: VIDEO },
    );
  });
  ...
  it("rejects anything that is not a YouTube target", () => {
    for (const input of [
      ...
      assert.equal(
        Either.isLeft(parseYoutubeTarget(input)),
        true,
        `expected ${input} to be rejected`,
      );
    }
...
describe("searchTarget", () => {
  it("keeps the query, trimmed", () => {
    assert.deepEqual(
      searchTarget("  planet earth "),
      Either.right({ query: "planet earth" }),
    );
  });
  ...
  it("refuses an empty query, which the app would silently drop", () => {
    for (const input of ["", "   ", "\t"]) {
      assert.equal(
        Either.isLeft(searchTarget(input)),
        true,
...
describe("launchPayload", () => {
  ...
  it("honours an app id override", () => {
    assert.equal(
      launchPayload({ videoId: "dQw4w9WgXcQ" }, "youtube.leanback.v6").id,
      "youtube.leanback.v6",
    );
  });
});
...
describe("tokenize", () => {
  it("splits plain whitespace-separated words", () => {
    ...
    assert.deepEqual(
      tokenize("key HOME UP UP ENTER"),
      Either.right(["key", "HOME", "UP", "UP", "ENTER"]),
    );
  });
  ...
  it("honours a backslash escape", () => {
    assert.deepEqual(
      tokenize("toast Hi\\ there"),
      Either.right(["toast", "Hi there"]),
    );
    assert.deepEqual(
      tokenize('toast "she said \\"hi\\""'),
      Either.right(["toast", 'she said "hi"']),
    );
  });

  it("does not un-escape inside single quotes", () => {
    assert.deepEqual(
      tokenize("toast 'a\\backslash'"),
      Either.right(["toast", "a\\backslash"]),
    );
  });
