...
import { LaunchPoints, Uri, VolumeStatus } from "../src/sdk/index.js";
...
describe("handshake", () => {
	it("pairs, and stores the key the TV hands back", async () => {
		const key = await run(
			Effect.scoped(Effect.map(connect(), (t) => t.clientKey)),
		);
...
describe("requests", () => {
	it("reads the volume through the schema", async () => {
		const status = await run(
			withTv((t) => t.requestAs(Uri.getVolume, VolumeStatus)),
		);
	...
	it("decodes list responses", async () => {
		const { launchPoints } = await run(
			withTv((t) => t.requestAs(Uri.listApps, LaunchPoints)),
		);
	...
	it("deep-links YouTube through the launcher", async () => {
		...
		const result = await run(
			withTv((t) =>
				t.request(
					Uri.launch,
					launchPayload({ videoId: "dQw4w9WgXcQ", startSeconds: 90 }),
				),
			),
		);
		assert.equal(
			...
			"string",
			"a launch reply carries a session id",
		);
	...
	it("surfaces an error frame as SsapFailed", async () => {
		const result = await run(
			Effect.either(withTv((t) => t.request("ssap://does/not/exist"))),
		);
	...
	it("treats returnValue:false as a failure and keeps the TV's reason", async () => {
		const result = await run(
			Effect.either(
				withTv((t) => t.request(Uri.openChannel, { channelNumber: "999" })),
			),
		);
		assert.equal(result._tag, "Left");
		if (result._tag === "Left") {
	...
	it("drops frames that are not JSON instead of crashing the listener", async () => {
		...
		if (updates._tag === "Success") {
			...
			assert.deepEqual(
				values,
				[41, 55],
				"the garbage frame should be skipped, not fatal",
			);
		}
	});
});
...
describe("transport resolution", () => {
		...
		Effect.runPromise(
			Effect.provide(
				...
				Session.layer({
					...
					Layer.provide(Settings.Default),
	...
	it("uses the transport the TV was paired over when no flag is given", async () => {
		await withSession(Option.some(true), (session) =>
			session.rememberKey(host, "key-secure"),
		);
		assert.equal(
	...
	it("forgets ssl when the TV is paired again over the plain port", async () => {
		await withSession(Option.some(false), (session) =>
			session.rememberKey(host, "key-plain"),
		);
		assert.equal(
			await withSession(Option.none(), (session) => session.wsUrl),
			`ws://${host}:3000`,
...
describe("connection failures", () => {
	it("reports an unreachable TV rather than hanging", async () => {
		...
		const result = await Effect.runPromise(
			Effect.provide(
				Effect.either(Effect.scoped(connect({ announcePairing: false }))),
				isolated,
			),
		);
