...
import {
	makeLineReader,
	makeScriptReader,
} from "../src/services/LineReader.js";
...
const makeStubReader = () => {
	...
	return {
		reader,
		type: (line: string) =>
			Effect.runPromise(Queue.offer(queue, Option.some(line))),
...
const withFakeTv = async (
	body: (ctx: {
		readonly tv: FakeTv;
		readonly run: <A, E>(
			effect: Effect.Effect<A, E, ReplContext>,
		) => Promise<A>;
	...
	const run = <A, E>(effect: Effect.Effect<A, E, ReplContext>) =>
		Effect.runPromise(
			Effect.provide(effect, runtime) as Effect.Effect<A, E, never>,
		);

	const startRepl = (reader: LineReader) =>
		Effect.runFork(
			Effect.provide(runRepl(reader), runtime) as Effect.Effect<
				void,
				unknown,
				never
			>,
	...
	const joinFiber = (fiber: ReturnType<typeof Effect.runFork>) =>
		Effect.runPromise(
			Fiber.join(fiber as Fiber.RuntimeFiber<unknown, unknown>),
		);
...
describe("repl", () => {
	it("keeps one connection and one handshake across many lines", async () => {
		await withFakeTv(async ({ tv, runRepl: start, join }) => {
			...
			assert.ok(
				tv.promptCount() <= 1,
				"only the first connect against a fresh config dir prompts",
			);
	...
	it("keeps the prompt after bad input, without opening another connection", async () => {
		await withFakeTv(async ({ tv, runRepl: start, join }) => {
			...
			assert.equal(
				tv.openConnections(),
				1,
				"bad or blank input must not open a second connection",
			);
			assert.equal(
				tv.registerPayloads.length,
				1,
				"bad or blank input must not trigger a re-handshake",
			);
...
describe("end to end", () => {
	...
	it("says nothing about replies unless VERBOSE is set", async () => {
		const { code, stdout, stderr } = await pipeScript(
			"volume\nkey HOME\nexit\n",
		);
	...
	it("echoes each reply the TV sends under VERBOSE, on stderr so stdout stays pipeable", async () => {
		...
		assert.match(
			stderr,
			new RegExp(`← ${Uri.getVolume}\\s+\\{"returnValue":true,"volume":\\d+`),
		);
		assert.doesNotMatch(
			stdout,
			/←/,
			"the echo belongs to the repl's chrome, not to the command",
		);
	});
	...
	it("carries on past a failed line and still exits 0 — stopping is `lgtv run`'s job", async () => {
		...
		assert.match(
			stdout,
			/power\s+unknown/,
			"the line after the failure still runs",
		);
	});
});
...
describe("makeScriptReader", () => {
	it("hands over every line once, then end of input", async () => {
		...
		assert.deepEqual(
			third,
			Option.none(),
			"an exhausted script ends the loop, like Ctrl-D",
		);
	});
});
...
describe("lgtv run", () => {
	...
	it("runs the whole sequence over one connection, with nothing on stdin", async () => {
		...
		assert.doesNotMatch(
			stderr,
			/Type a command/,
			"a script has no prompt to greet",
		);
	});
	...
	it("renders a failure as JSON under --json, like every other command", async () => {
		const child = spawn(
			...
			{
				stdio: ["ignore", "pipe", "pipe"],
				env: { ...process.env, LGTV_CONFIG_DIR: configDir },
			},
		);
		let stderr = "";
		child.stderr.on("data", (chunk: Buffer) => {
...
describe("tokenize (repl integration sanity)", () => {
	it("is what the repl loop uses to split a line before dispatch", () => {
		assert.deepEqual(
			tokenize("volume set 20"),
			Either.right(["volume", "set", "20"]),
		);
	});
});
