...
export const startFakeTv = async (
  ...
  server.on("connection", (socket) => {
    socket.on("message", (raw) => {
      ...
      if (frame.type === "register") {
        ...
        const alreadyPaired =
          typeof offered === "string" && knownKeys.has(offered);
        ...
        switch (pairing) {
          ...
          case "reject":
            send(socket, {
              type: "error",
              id: frame.id,
              error: "403 user denied the request",
            });
            return;
      ...
      const reply = (payload: Record<string, unknown>) =>
        answer({ returnValue: true, ...payload });
  ...
  const pushRaw = (
    make: (subscription: { uri: string; id: string }) => string,
  ) => {
  ...
  return {
    ...
    pushWrongShape: () =>
      pushRaw(({ id }) => JSON.stringify({ type: 42, id, payload: {} })),
    pushWithoutId: () =>
      pushRaw(() =>
        JSON.stringify({ type: "response", payload: { volume: -1 } }),
      ),
    pushUnknownId: () =>
      pushRaw(() =>
        JSON.stringify({
